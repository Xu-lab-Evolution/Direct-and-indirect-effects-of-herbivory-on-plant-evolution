...
**Note:** This R Markdown document is used to generate **Figure 2B** in manuscript, **FigureS1D**, **Table S4** and **Table S5** in supplementary.
...
## Plot 2B: macroalgae over time

```{r,echo=FALSE, message=FALSE, warning=FALSE}
px2D <- ggplot(mdata.plot, aes(x = transform_x(timepoint), y = macroalgae_av, color = treatment)) +
...
model <- lmer(macroalgae_logit~ treatment * timepoint + (1|pond), data = df1)
Anova(model,test="F")
summary (model)
par(mfrow = c(1, 2))
plot(model)                   
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
pxS1D <- ggplot(mdata.plot, aes(x = transform_x(timepoint), y = chla_av, color = treatment)) +
  theme_bw(base_size = 14) +
...
df1$timepoint_c <- scale(df1$timepoint, center = TRUE, scale = FALSE)

model <- lmer(Chlorophyll_a ~ treatment * timepoint_c + (1|pond), data = df1)
Anova(model,test="F")
summary(model)
par(mfrow = c(1, 2))
plot(model)                   
...
df2$timepoint_c <- scale(df2$timepoint, center = TRUE, scale = FALSE)

model <- lmer(Chlorophyll_a ~ treatment * timepoint_c + (1|pond), data = df2)
Anova(model,test="F")
summary(model)
