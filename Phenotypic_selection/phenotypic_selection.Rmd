...
This R Markdown document analyzes the phenotypic selection. The analyses support the generation of **Figure 4** (main text) and **Figures S7-12**, **Table S7** (supplementary material) in the manuscript.
...
sel_cols  <- c(Direct_Effect   = "#8CBA5E",          # for dynamics plot
               Indirect_Effect = "#A87C7C")
```
...
if (nrow(dyn_dat) > 0) {
  p_dyn <- ggplot(dyn_dat, aes(
    ...
    labs(
      x = "Year",
      y = "Selection estimate ± SE",
      title = "Temporal dynamics of direct vs. indirect herbivory effects",
      subtitle = "Metabolites with significant selection in any year"
    ) +
