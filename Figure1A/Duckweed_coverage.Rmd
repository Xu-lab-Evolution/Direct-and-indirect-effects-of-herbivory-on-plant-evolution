...
**Note:** This R Markdown document is used to generate **Figure 1A** in the manuscript.
...
We load the data (data.txt, tab-separated) and calculate the mean and standard error of surface coverage, grouped by year, timepoint, and treatment.
...
This function is used to transform the x-axis (timepoints) for the plot.
...
## Generate the Plot
The plot below shows duckweed coverage over time with error bars, colored by treatment, and includes several custom segments and shaded regions.
...
px1 <- ggplot(mdata.plot, aes(x = transform_x(timepoint), y = coverage_av, color = treatment)) +
  ...
  scale_color_manual(name = "Legend", values = c("#00afbb", "#e41a1c"),
...
We fit a linear mixed-effects model with treatment as a fixed effect and random factors for timepoint within pond.

```{r,echo=FALSE, message=FALSE, warning=FALSE}
df1 <- df %>%
  filter(timepoint %in% c(7, 11, 13))

model <- lmer(surface_coverage ~ treatment + (timepoint|pond), data = df1)
Anova(model, test = "F")
```
...
model <- lmer(surface_coverage ~ treatment + (timepoint|pond), data = df2)
Anova(model, test = "F")
```
