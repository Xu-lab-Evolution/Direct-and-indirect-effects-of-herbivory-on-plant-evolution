---
title: "Metabolites-fitness associations"
author: "Sara Nouere"
output: html_document
---

This R Markdown document analyzes the metabolites-fitness associations. The analyses support the generation of **Figure 5** and **Table 1** in the main text and **Figures S6-11**, and **Table S11 and S12** in the supplementary material in the manuscript.
...
calc_effects <- function(df, met) {
  ...
  best <- list(m = m_raw, d = d_raw, tf = "raw")

  if (!inherits(m_raw, "merMod") && inherits(m_log, "merMod")) {
    best <- list(m = m_log, d = d_log, tf = "log1p")
  }

  if (!inherits(best$m, "merMod")) return(NULL)
...
# Representative diagnostic example (same procedure applied to 2024 models)
diag_model <- all_res[["2023"]][["Chlorogenic.acid"]]$model

# Residual diagnostics
par(mfrow = c(1, 2))
plot(diag_model)                
qqnorm(resid(diag_model))      
qqline(resid(diag_model))
par(mfrow = c(1, 1))
