...
px2A <- ggplot(mdata.plot, aes(x = transform_x(timepoint), y = coverage_av, color = treatment)) +
...
model <- lmer(surface_coverage ~ treatment * timepoint_c +(1|pond), data = df1)
Anova(model, test = "F")
summary(model)
par(mfrow = c(1, 2))
plot(model)                   
...
model <- lmer(surface_coverage ~ treatment * timepoint_c + (1|pond), data = df2)
Anova(model, test = "F")
pairs(emmeans(model, ~ treatment | timepoint_c))
summary(model)
par(mfrow = c(1, 2))
plot(model)                   
qqnorm(resid(model))           
...
## In the main text: effect of snail herbivory on duckweed surface coverage in 2024
