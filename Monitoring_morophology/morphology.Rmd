...
**Note:** This R Markdown document is used to generate **Figure S2A** and **Figure S2B** in the supplementary.
...
```{r,echo=FALSE, message=FALSE, warning=FALSE}
pxS2A <- ggplot(mdata.plot, aes(x = transform_x(timepoint), y = weight_av, color = treatment)) +
...
### Figure S2B
## Plot S2B: area per frond

```{r,echo=FALSE, message=FALSE, warning=FALSE}
pxS2B <- ggplot(mdata.plot, aes(x = transform_x(timepoint), y = area_av, color = treatment)) +
  theme_bw(base_size = 14) +
